...
-- +goose Down
-- +goose StatementBegin
DELETE FROM seichies WHERE content_id IN (SELECT content_id FROM contents WHERE content_name IN ('君の名は。', 'けいおん！', 'らき☆すた', 'ゆるキャン△', '氷菓'));
DELETE FROM places WHERE place_id IN (1, 2, 3, 4, 5);
DELETE FROM contents WHERE content_id IN (1, 2, 3, 4, 5);
-- +goose StatementEnd 