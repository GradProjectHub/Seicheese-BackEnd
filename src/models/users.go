...
// Point pointed to by the foreign key.
...
// CheckinLogs retrieves all the checkin_log's CheckinLogs with an executor.
...
// PointLogs retrieves all the point_log's PointLogs with an executor.
func (o *User) PointLogs(mods ...qm.QueryMod) pointLogQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`point_logs`.`user_id`=?", o.UserID),
	)

	return PointLogs(queryMods...)
}
...
func (o *User) Seichies(mods ...qm.QueryMod) seichyQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`seichies`.`user_id`=?", o.UserID),
	)

	return Seichies(queryMods...)
}

// LoadPoint allows an eager lookup of values, cached into the
...
func (userL) LoadPoint(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`points`),
		qm.WhereIn(`points.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load Point")
	}

	var resultSlice []*Point
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice Point")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for points")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for points")
	}
...
// LoadCheckinLogs allows an eager lookup of values, cached into the
...
func (userL) LoadCheckinLogs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load checkin_logs")
	}

	var resultSlice []*CheckinLog
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice checkin_logs")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on checkin_logs")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for checkin_logs")
	}
...
// LoadPointLogs allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPointLogs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load point_logs")
	}

	var resultSlice []*PointLog
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice point_logs")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on point_logs")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for point_logs")
	}
...
// SetPoint of the user to the related item.
// Sets o.R.Point to related.
...
// AddCheckinLogs adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CheckinLogs.
...
// AddPointLogs adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.PointLogs.
// Sets related.R.User appropriately.
func (o *User) AddPointLogs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PointLog) error {
	...
	if o.R == nil {
		...
		o.R.PointLogs = append(o.R.PointLogs, related...)
	}
...
func (o *User) AddSeichies(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Seichy) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.UserID, rel.SeichiID}
